...
public class GestorKeep {
    ...
    public static void updateKeepAndroid(Keep aux){
         Configuration configuration = new Configuration().configure();
        StandardServiceRegistryBuilder builder = new StandardServiceRegistryBuilder().
                applySettings(configuration.getProperties());
        SessionFactory factory = configuration.buildSessionFactory(builder.build());
        Session sesion = factory.openSession();
        sesion.beginTransaction();
        String hql = "from Keep where idAndroid = :idAndroid";
        Query q = sesion.createQuery(hql);
        q.setInteger("idAndroid", aux.getIdAndroid());
        List<Keep> keeps = q.list();
        Keep k = keeps.get(0);
        k= (Keep) sesion.get(Keep.class, k.getId());
        k.setContenido(aux.getContenido());
        k.setEstado(aux.getEstado());
        sesion.getTransaction().commit();
        sesion.flush();
